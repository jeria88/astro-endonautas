const API_URL = "https://api.endonautas.cl";

export async function onRequestPost({ request, env }) {
  const apiKey = env.CONTENT_STUDIO_API_KEY;
  if (!apiKey) return Response.json({ error: "CONTENT_STUDIO_API_KEY no configurado" }, { status: 500 });

  let slug, approved_selections;
  try {
    const body = await request.json();
    slug = body.slug;
    approved_selections = body.approved_selections;
    if (!slug || !Array.isArray(approved_selections) || !approved_selections.length) {
      throw new Error("faltan campos");
    }
  } catch {
    return Response.json({ error: "Body inválido — se espera { slug, approved_selections }" }, { status: 400 });
  }

  const res = await fetch(`${API_URL}/api/articles/${encodeURIComponent(slug)}/approve`, {
    method: "POST",
    headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify({ approved_selections }),
  });
  if (!res.ok) return Response.json({ error: await res.text() }, { status: res.status });
  return Response.json(await res.json());
}
